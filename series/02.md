...
| 項目 | 内容 |
| --- | --- |
| **Key Concepts** | ノルム、距離の集中、直交性、次元の呪い |
| **Key Metaphor** | 三つの顔を持つ数値（意味・確信度・ノイズ）、赤道への集中 |
| **Key Question** | ノルムは何を測っていたのか？なぜ分離すべきだったのか？ |
| **Key Insight** | 高次元の「呪い」は、見方を変えれば「祝福」にもなる |
...
Word2Vec（Mikolov et al., 2013）を例に考えよう。学習後の単語ベクトルを観察すると、**高頻度語ほどノルムが大きい**傾向がある。
...
深層学習の初期（2010年代前半）には、表現ベクトルを正規化せずに使うことが一般的だった。コサイン類似度を使う場合でも、計算時に正規化するだけで、表現自体は非正規化のままだった。

この設計には、以下の問題がある：

1. **類似度とノルムの干渉：** 二つのベクトルの類似度を測る際、ノルムの違いが「意味の違い」と混同される
...
距離の集中は、**最近傍探索（kNN）** に深刻な影響を与える。

理想的には、「近い点」と「遠い点」を明確に区別したい。しかし、高次元では：

$$\frac{d_{\max} - d_{\min}}{d_{\min}} \to 0 \quad (d \to \infty)$$

最遠点と最近点の距離比が1に近づく。すべての点が「だいたい同じ距離」になり、「近さ」の概念が曖昧になる。

これが、第1回で触れた「kNNの高次元での困難」の数学的背景である。
...
直交するベクトルは「互いに干渉しない」。高次元空間には、内積が小さい（ほぼ直交する）ベクトルを**指数的に多く**配置できる。

> [!NOTE]
> **球面符号との関係：** 「内積が $\epsilon$ 以下のベクトルをいくつ配置できるか」は、球面符号（spherical code）の問題として研究されている。$d$ 次元で内積 $\epsilon$ 以下のベクトルは、$\exp(c \cdot d)$ 個程度配置できることが知られている（$c$ は $\epsilon$ に依存する定数）。
...
カプセルネットワークは、理論的には美しいアイデアだが、実用上の課題が多い：

- **計算コスト：** 動的ルーティングが計算量を増やす
- **スケーラビリティ：** 大規模データセットへの適用が困難
- **性能：** CNNやTransformerに比べて、ベンチマークでの優位性が限定的

> [!NOTE]
> **カプセルネットワークの意義：** 実用的な成功は限定的だったが、「向きと長さの分離」という問題提起は重要である。この発想は、後のCosFace/ArcFace（第5回）や、nGPTの正規化設計に影響を与えている可能性がある。
