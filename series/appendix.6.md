...
ここで $L_{min}$ は真の分布 $q(x)$ に対する期待損失（KLダイバージェンス）の最小値であり、モデルが真の分布を実現可能なら0になる（実務的には「学習ロスの下限」と読んでよい）。 $n$ はデータ数、 $d$ はパラメータ数である。

正則モデルの第2項の係数はパラメータ数の半分 $d/2$ であり、BIC（ベイズ情報量基準）に一致する。パラメータが増えるほどペナルティが増え、汎化性能は悪化すると予測される。

一方、特異モデルでは係数 $\lambda$ はパラメータ数 $d$ とは一致せず、代わりに **実対数正準閾値（RLCT: Real Log Canonical Threshold）** と呼ばれる幾何学的な量になる。 $m$ は学習ゼータ関数の極の**重複度（multiplicity）**であり、モデルによっては $m > 1$ となって $\log \log n$ 項が寄与する。
