...
$$F(D_n) \approx n L_{min} + \lambda \log n - (m-1) \log \log n + O(1)$$

ここで $L_{min}$ は学習ロス、 $n$ はデータ数である。重要なのは第2項の係数 $\lambda$ だ（第3項以降は通常省略されるが、モデルによっては $\log \log n$ 項や定数項が寄与する場合がある）。
