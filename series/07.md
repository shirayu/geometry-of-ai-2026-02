...
本回では、この固定を解放する。**解像度を上げる**ことで、概念を「点（Point Estimate）」としてだけでなく、不確実性を含んだ「分布（Distribution）」として捉え直す。これにより、今まで見えなかった「迷い」や「知らなさ」を可視化できる。これは、「分かりません」と言えるAIへの第一歩である。
...
重要なのは、従来の点埋め込みがこの枠組みから外れているわけではない、という点である。点埋め込みは、**確信度パラメータを無限大に固定した特殊ケース**と見なすことができる。分布表現は、この固定を解放し、確信度を学習可能なパラメータとして扱う一般化である。

> [!IMPORTANT]
> **設計としての分離：** この「意味と確信度の分離」は、データから自動的に得られるものではない。vMF分布を使い、 $\kappa$ （集中度）を明示的に学習する設計を採用して初めて実現する。単に $L_2$ 正規化するだけでは、点埋め込みのままであり、不確実性は表現されない。言い換えれば、従来の手法は $\kappa = \infty$ （完全な確信）を暗黙の前提としていたと解釈できる。

### [コラム] Zipf則の幾何学的正体 ～なぜ「頻出語」は広がるのか～

<details>
<summary>要約: 高頻度語の多義性は、空間上の分散として表れる。</summary>
...
1. **Zipfの意味-頻度則**：頻度が高い単語ほど、多くの意味を持つ（多義性が高い）。
2. **文脈的多様性（Contextual Diversity）の本質**：頻度が高いから意味が広いのではなく、**「多様な文脈で使われる」から意味が広がる**のである。

これを本講義の「プラネタリウム」のメタファーで捉え直すと、どうなるか？

**多様な文脈 = 広い視野角**：様々な文脈に出現する単語は、球面上の「一点」ではなく、 **「広い領域」** を占有しなければならない。

**幾何学的帰結**：つまり、高頻度語（高文脈多様性）は、 **集中度 $\kappa$ が低くなりやすい（分散が大きくなる傾向がある）** と期待される。
...
**従来のアプローチとの接続**：第2回で見た「頻度が高い単語は長いベクトル」という観察（主に静的埋め込みで見られる傾向）は、この文脈的多様性を「ノルム」という形で暗黙的に捉えようとしていたと解釈できる。しかし、ノルムは方向情報と混在するため解釈が困難だった。分布埋め込み（ $\kappa$ の導入）は、この混在を解消し、 **方向＝意味、 $\kappa$ ＝確信度（文脈的多様性の逆数に対応、ただし直接的な等価ではない）** という明確な分離を実現する。
...
標準正規分布 $\mathcal{N}(\mathbf{0}, \mathbf{I})$ の確率密度関数は原点で最大値をとる。しかし、高次元では**密度が最大の場所にサンプルが集まるとは限らない**。次元 $d$ が大きくなると、半径 $r$ の球殻の体積が $r^{d-1}$ に比例して急増するため、密度と体積の積——すなわち確率質量——は原点から離れた $r \approx \sqrt{d}$ 付近の薄い殻に集中する（concentration of measure）。これが**典型集合**と呼ばれる領域であり、特に半径方向の薄い帯として見た呼び方が**典型帯**である。
...
> [!TIP]
> **密度と体積の綱引き：なぜ $\sqrt{d}$ なのか**
>
> ガウス分布の密度（高さ）は原点で最大だが、高次元空間ではそこにサンプルは集まらない。
> これを理解するには、 **「半径 $r$ の薄皮（厚さ $dr$ ）の中に落ちる確率」** を考える必要がある。
>
> $$P(r)dr \propto \underbrace{r^{d-1}} _{\text{広がる力（表面積）}} \times \underbrace{e^{-r^2/2}} _{\text{引く力（密度）}} dr$$
>
> 1. $r \approx 0$ （原点付近）: 密度は最大だが、半径 $r$ が小さすぎて $r^{d-1} \approx 0$ となり、確率はほぼゼロ（場所がない）。
> 2. $r \to \infty$ （遠方）: 場所は広いが、密度 $e^{-r^2/2}$ が急激にゼロになり、確率もゼロ（空気がない）。
>
> **均衡点の導出**: この確率 $P(r)$ が最大になる場所は、対数をとって $r$ で微分することで求まる。
>
> $$\frac{d}{dr} \left( (d-1)\ln r - \frac{r^2}{2} \right) = \frac{d-1}{r} - r = 0 \implies r = \sqrt{d-1} \approx \sqrt{d}$$
> つまり、高次元のサンプルは原点にはおらず、この半径 $\sqrt{d}$ 付近の非常に薄い「殻」——**典型集合（Typical Set）**——に張り付いている。VAEで事前分布と事後分布が乖離する「Typical Set Mismatch」も、この幾何学的な力が引き起こす必然的な現象である。
...
第3回で「プラネタリウム」として導入した球面表現は、この問題に対する幾何学的な回答である。単位球面 $\mathbb{S}^{d-1}$ 上では、すべての点が原点から等距離にあり、半径方向の自由度がそもそも存在しない。情報はすべて方向に集約される。

第2回、第3回で見た $L_2$ 正規化の効果も、この「半径の消去」によるものであった。ベクトルを単位球面に配置することは、幾何学的には「半径方向の情報を持たない」と宣言することである。点埋め込みの視点で言えば、これは **「不確実性（分散）の情報を持たない」——つまり確信度を無限大に固定する** ——ことに相当すると見なせる。
...
球面上での分布表現として**vMF分布**を選ぶことは、この設計思想の自然な帰結である。vMF分布は、従来の点埋め込み（ $\kappa = \infty$ ）を特殊ケースとして包含しつつ、確信度という新たな次元を解放する。

| | VAE（ガウス） | vMF（球面） | 従来の点埋め込み |
| --- | --- | --- | --- |
| 空間 | $\mathbb{R}^d$ （ユークリッド空間） | $\mathbb{S}^{d-1}$ （単位球面） | $\mathbb{S}^{d-1}$ （単位球面） |
| 正則化の方向 | 標準正規への引力（ $\boldsymbol{\mu} \to \mathbf{0}, \sigma \to 1$ ） | 球面上の一様分布への引力 | （正則化なし） |
| 半径方向の偏り | あり（典型帯 $\sqrt{d}$ に集中） | なし（半径の自由度が存在しない） | なし（ノルム1に固定） |
| 確信度の表現 | 分散 $\sigma^2$ | 集中度 $\kappa$ | $\kappa = \infty$ （暗黙の仮定） |
| 意味と確信度の分離 | ノルムに意味が混入しうる | 方向＝意味、 $\kappa$ ＝確信度 | 分離不可（確信度固定） |
...
> [!IMPORTANT]
> **包含としての一般化：** vMF分布は、従来の点埋め込みを否定するものではなく、それを $\kappa = \infty$ という極限状態として包含する一般化である。点埋め込みは「計算効率のために確信度を捨象した圧縮形」であり、分布埋め込みは「その圧縮を解き、本来の『広がり』を取り戻した高解像度版」と位置づけられる。
...
**重要な認識**：我々がこれまで使ってきた従来の点埋め込み（正規化して使う場合のWord2Vec、正規化されたTransformer埋め込みなど）は、この表の最後の行——すなわち $\kappa = \infty$ という極限状態——を暗黙のうちに仮定していたと解釈できる。単位球面上の一点に配置することは、分布埋め込みの視点から振り返れば、「この方向に確信度100%で存在する」と宣言することに相当すると見なせる。

分布埋め込みとは、この $\kappa$ を無限大に固定せず、**データに合わせて学習可能なパラメータとして解放する**試みである。これにより、モデルは「自信がない」状態を表現できるようになる。

プラネタリウムのメタファーで言えば：

- 大きい $\kappa$ ：「あの星座しか見えない」（晴れた夜空、高確信）
- 小さい $\kappa$ ：「複数の星座が重なって見える」（霧の夜、低確信）
- $\kappa = \infty$ ：「レーザーポインタで一点を指す」（従来の点埋め込み）
...
> **Bessel関数 $I_\nu$ は何を「調整」しているのか?**
>
> 数式に現れる第一種修正Bessel関数 $I_\nu(\kappa)$ は、確率分布の整合性を保つための **「バランサー」** である。
>
> 確率分布は、空間全体での積分値（合計）が必ず「1」でなければならない。 指数部 $\exp(\kappa \boldsymbol{\mu}^\top \mathbf{x})$ を球面全体で積分した値（分配関数 $Z(\kappa)$ ）は、Bessel関数を用いて計算される。
>
> $$Z(\kappa) \propto \frac{I_{d/2-1}(\kappa)}{\kappa^{d/2-1}}$$
> 我々が必要とする正規化定数 $C_d(\kappa)$ は、この積分値の逆数 $1/Z(\kappa)$ である。
>
> - $\kappa \to 0$ のとき:
>     - 指数部は定数となり、積分値 $Z$ は球面の表面積（Surface Area）に一致する。結果として、正規化定数 $C$ は $1/\text{Area}$ となり、これは**一様分布**を意味する。
> - $\kappa$ が大きいとき:
>     - 分布が鋭くなり積分値 $Z$ が小さくなるため、正規化定数 $C$ は大きくなり、釣じりを合わせる。
>
> つまりBessel関数項は、**空間の広がりと山の鋭さに応じて、総和を1に保つための補正係数を計算している**のである。
...
第4回で登場した情報量 $-\log p(x)$ が「確実性からの遠さ」だったとすれば、エントロピーはその遠さの平均であり、分布全体が空間を占める「体積」に対応する：

$$H(P) = \mathbb{E}_{x \sim P}[-\log p(x)]$$

$\kappa$ との対応で言えば：

- $\kappa \to \infty$ （点埋め込みの極限）：霧が凝縮してゼロ体積の点に収束。エントロピー最小。
- $\kappa \to 0$ （一様分布の極限）：霧が球面全体に薄く広がる。エントロピー最大。
...
従来の点埋め込み（ $\kappa = \infty$ を暗黙の前提とする手法と見なせるもの）では、入力は必ずどこかの方向に射影される。未知の入力に対しても、**「ここです（確信度100%）」と無理やり断言する**構造になっている。星がない領域でも、最も近い星に関連付けられる。これは、不確実性を表現する自由度（ $\kappa$ パラメータ）を持たないことの帰結である。

> [!NOTE]
> **「確信度100%」の意味**：ここでの「確信度100%」は、分布埋め込みの視点から従来の点埋め込みを振り返った際の比喩的表現である。従来の点埋め込みが明示的に「確信度」を計算しているわけではなく、単に「不確実性を表現する次元を持たない」だけである。vMF分布として再解釈した場合に、 $\kappa \to \infty$ の極限に対応すると見なせる、という意味である。

幾何学的に言えば、点埋め込みは「場所（方向）」という一つの軸しか持たない。未知の入力が来たとき、その「場所」をどこかに配置せざるを得ないが、 **「霧の濃さ」という第二の軸** がないため、「ここだけど自信はない」という状態を表現できない。

この構造が、**Out-of-Distribution（OOD）入力に対する脆弱性**の一因である。

> [!NOTE]
> $\kappa$ **測定で検出できるハルシネーションの限界**
>
> 本回で扱う集中度 $\kappa$ による不確実性検出は、「**無知の捏造**」型のハルシネーション——知らないことを自覚できず、確率的に最もありそうな答えを返してしまう現象——に有効である。
>
> しかし、 $\kappa$ が高くても間違っている「**誤信念の表出**」型（訓練データの誤りを確信を持って学習してしまった状態）は、内在幾何に基づく指標だけでは検出できない。この問題の詳細は[Appendix 2: 多様体の純度問題](appendix.2.md)を参照されたい。

### 分布表現による解決策

分布表現（ $\kappa$ を学習可能にする設計）を使えば、OOD検知に有力なシグナルが得られる。

**低 $\kappa$ 領域の検出**：入力に対して推定される $\kappa$ が低い場合、モデルはその入力について「よく分からない」と言っている。これはOODの可能性を示唆する。

従来の点埋め込み（不確実性の次元を持たず、 $\kappa = \infty$ 相当）では：

- 未知の入力 → 「これは犬です（確信度100%）」という誤答（ハルシネーション）

分布埋め込み（ $\kappa$ を学習）では：

- 未知の入力 → 「場所は犬のあたりだが、霧のように薄くて自信がない（ $\kappa$ 小）」

この **「霧の濃さ（ $\kappa$ ）」という第二の軸** こそが、AIに謙虚さを与える。
...
> **ハルシネーション = 確信度（ $\kappa$ ）の情報がないまま、星座を強制的に結ぶ行為**

星がまばらな領域（学習データが少ない、または概念が曖昧な領域）で、モデルはそれでも何らかの出力を生成しなければならない。従来の点埋め込み（不確実性の次元を持たないため、分布埋め込みの視点では $\kappa = \infty$ 固定と見なせるもの）では、「分かりません」という選択肢がないため、確信度100%で誤った出力を生成してしまう。

分布埋め込みの視点で言えば、ハルシネーションは「 $\kappa$ が小さいべき領域で、無理やり大きな $\kappa$ を出力する」ことに相当する。逆に言えば、 $\kappa$ を適切に推定できれば、ハルシネーションのリスクを検知できる可能性がある。
...
分布表現を採用し、 $\kappa$ のような**回転不変統計量**を使えば、座標系に依存しない比較が可能になる場合がある。
...
| **典型帯** | 典型集合のうち半径 $\sqrt{d}$ 付近の薄い帯 | ガウス潜在空間の実効的な居住領域 |
...
従来の点埋め込みから分布埋め込みへの移行は、単なる技術的な拡張ではない。**「分かりません」と言える能力**をモデルに与えるという、質的な変化である。
...
**重要な認識**：点埋め込みは分布埋め込みと対立するものではなく、 $\kappa = \infty$ という極限ケースとして包含される。この視点により、従来の手法と新しい手法が統一的に理解できる。

### 視点の階層：「点」という言葉の多義性

本回では、「点」から「分布」への移行を扱った。しかし、最後に一つ重要な注意がある。**我々が学習した分布のパラメータ $(\boldsymbol{\mu}, \kappa)$ 自体は、パラメータ空間という『メタな空間』における一つの点である。**

これは混乱を招きやすいが、以下の3つの階層を区別すれば整理できる：

| 階層 | 実体 | 直感的イメージ | 不確実性の有無 |
| --- | --- | --- | --- |
| **レベル1：表現空間の点** | $\boldsymbol{h} \in \mathbb{S}^{d-1}$ | プラネタリウムの星（単一の方向） | なし（ $\kappa = \infty$ ） |
| **レベル2：表現空間の分布** | $p(\boldsymbol{h}; \boldsymbol{\mu}, \kappa)$ | 星の周りの霧（広がりを持つ分布） | あり（ $\kappa$ が有限） |
| **レベル3：統計多様体の点** | $(\boldsymbol{\mu}, \kappa)$ | その霧の形を指定する座標 | （次の階層で議論） |

従来の点埋め込み（レベル1）を分布埋め込み（レベル2）に拡張したことで、解像度が上がった。しかし、その分布を指定するパラメータ $(\boldsymbol{\mu}, \kappa)$ は、より高次の空間（統計多様体）における「点」である。
...
> - **KL・正規化定数**：高 $\kappa$ ・高次元での漸近近似を使用。 $\kappa$ が小さい領域では誤差が大きくなる可能性があるため、厳密計算が必要な場合はscipy.special.ive等を使用すること
...
> [!CAUTION]
> **以下の実装は高 $\kappa$ ・高次元での漸近近似に基づく。** $\kappa$ が小さい領域（まさに不確実性が高く重要な領域）では近似誤差が大きくなる。厳密な計算が必要な場合は、scipy.special.ive（スケール付きBessel関数）等を使った安定実装を検討すること。詳細はDiethe (2015) を参照。
...
> [!IMPORTANT]
> $\kappa$ **閾値によるOOD検知は、必ず校正（calibration）が必要。** モデルが出力する $\kappa$ は、学習データやアーキテクチャに依存し、そのままでは「確率的に正しい確信度」を表さないことが多い。運用前に検証セットで以下を行うこと：
>
> - 閾値の選定（Precision-Recallトレードオフの確認）
> - 温度スケーリング等による $\kappa$ の校正
> - 分布内/分布外の $\kappa$ 分布の可視化
...
- Nagata et al. (2025). [A New Formulation of Zipf's Meaning-Frequency Law through Contextual Diversity](https://aclanthology.org/2025.acl-long.744/). *ACL 2025*.
    - Zipfの意味-頻度則を文脈的多様性（Contextual Diversity）の観点から再定式化。頻出語が埋め込み空間で広い分散を持つ理論的根拠を提供。本回では、vMF分布における低 $\kappa$ （広がり）の言語学的正当性を説明するコラムで言及。
...
> **発展的議論：データの純度問題**
>
> 本回の不確実性推定（低 $\kappa$ の検出）では対処できない、「高 $\kappa$ だが誤った方向」を指すハルシネーションについては、[Appendix 2: 多様体の純度問題](appendix.2.md)で扱う。幾何学が「データの真偽」に対して原理的に中立である理由と、外部検証が必要となる構造を説明している。
