...
> [!IMPORTANT]
> **設計の徹底が本質：** 単に `F.normalize` を呼ぶだけでは、球面上の学習にはならない。行列積や加算などの演算を行うと、結果は一般に球面から外れる。nGPTは、**更新で球面から外れたら正規化で戻す**（これを多様体最適化では「retraction」と呼ぶ）ことで、表現が常に球面上にあることを保証する。この「球面制約の維持」が、単なる正規化との違いである。
...
1. **数値安定性の課題：** NormFaceでも指摘されているように、正規化を伴う学習では勾配のスケーリングに注意が必要だった（Wang et al., 2017）
