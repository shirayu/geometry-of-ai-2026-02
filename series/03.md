...
> [!IMPORTANT]
> **設計の徹底が本質：** 単に `F.normalize` を呼ぶだけでは、球面上の学習にはならない。演算（加算、行列積など）を行うと、結果は一般に球面から外れる。nGPTは、**各演算後に再正規化する**ことで、表現が常に球面上にあることを保証する。この「徹底」が、単なる正規化との違いである。
...
### 歴史的文脈：なぜ2010年代にはできなかったか

球面制約という発想自体は新しくない。しかし、2010年代前半には実用化が困難だった：

1. **計算の不安定性：** 正規化を繰り返すと、勾配が消失または爆発しやすかった
2. **最適化技術の未成熟：** 球面制約を維持しながら効率的に最適化する手法がなかった
3. **ハードウェアの制約：** 正規化のオーバーヘッドが相対的に大きかった
...
> [!NOTE]
> **技術の連鎖：** nGPTは「球面制約」という古いアイデアを、現代の安定化技術の上に再構築したものと見ることができる。単独の発明というより、技術的条件が整ったことで可能になった設計である。
...
クラス $k$ に対応する平均方向を $\boldsymbol{\mu}_k$、入力の埋め込みを $\mathbf{x}$（ともに単位ベクトル）とすると：
