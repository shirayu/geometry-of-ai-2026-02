...
Chain of Thought（CoT）は、近年の大規模言語モデル（LLM）の推論能力を飛躍的に向上させた技術である。「ステップバイステップで考えよう」という単純なプロンプトが、なぜこれほど効果的なのか。

従来の理解では、CoTは「中間ステップを出力することで、モデルが複雑な推論を分解できる」と説明される。しかし、幾何学的な視点から見ると、より深い構造が見えてくる。

**推論とは、表現空間上を移動する「旅」である。**
...
CoTの本質は、この**連続的な思考過程を、中間的な離散トークンで「足場」を作りながら進める**ことにある。

### 足場としての中間トークン

なぜ中間トークンが「足場」として機能するのか。

**1. 作業記憶の外部化**：Transformerは固定長のコンテキストを持つ。複雑な推論では、中間結果を「覚えておく」必要がある。中間トークンとして出力することで、この情報がコンテキストに残り、後続の推論で参照できる。

**2. 軌跡の安定化**：連続空間での長い軌跡は、途中で「迷子」になりやすい。中間トークンは、軌跡上の「チェックポイント」として機能し、正しい方向への進行を確認する。

**3. 探索空間の構造化**：中間ステップを出力することで、探索空間が階層的に構造化される。大きな問題が小さな部分問題に分解され、各部分問題は独立に解きやすくなる。

> [!IMPORTANT]
> **CoTは単なる「説明」ではない：** CoTの効果は、「モデルが思考過程を説明している」だけでは説明できない。実際には、中間トークンの生成がモデルの内部状態に影響を与え、後続の推論を変化させている。これは「足場」の比喩が示唆する、より深い計算論的効果である。
...
| Test-time スケーリング | 推論 | 推論時の計算量（トークン数、探索幅） |

### 計算量を増やす方法

推論時の計算量を増やす方法はいくつかある。

**1. より長い思考（CoT）**：中間ステップを多く生成することで、より複雑な推論が可能になる。

**2. 複数回の試行（Self-consistency）**：同じ問題に対して複数回推論を行い、多数決や検証で最終回答を決定する。

**3. 探索の拡張（Tree of Thoughts）**：複数の思考経路を並行して探索し、最も有望な経路を選択する。

**4. 検証と修正（Self-reflection）**：生成した回答を自己検証し、必要に応じて修正する。

### 幾何学的解釈

Test-time compute scalingを幾何学的に解釈すると、以下のようになる。
...
def visualize_cot_comparison():
    ...
    # 直接経路も点線で表示
    ax3.plot([start[0], end[0]], [start[1], end[1]], [start[2], end[2]], "r--", linewidth=1, alpha=0.5, label="Direct (blocked)")
