...
| **位相多様体** (Topological Manifold) | 連続性のみを前提にした多様体。穴や連結性など、位相的性質を扱う土台。 |
| **可微分多様体** (Differentiable Manifold) | 微分可能な座標変換を備えた多様体。接空間や微分を定義できる。 |
| **多様体仮説** | 高次元データの実質的な構造は、低次元の多様体上に集中しているという仮説。 |
| **リーマン多様体** | 距離、角度、曲率を定義するために「リーマン計量」を導入した多様体。ただし計量が当然の前提の話では単に「多様体」とよぶこともある。 |
| **微分** (Differential) | 関数の局所的な変化を一次近似で捉える道具。可微分多様体で方向変化を扱う基礎。 |
...
| **双曲空間／双曲多様体** (Negative Curvature) | <a id="kw-hyperbolic-space"></a>負の曲率を持つ空間。指数関数的な広がりを持ち、階層構造の表現に適している。 |
...
曲がった多様体を「平らな座標」に展開するための、2000年前後に確立された古典的アプローチ。

| キーワード | 説明 |
| --- | --- |
| **多様体学習** | 高次元空間に埋め込まれた低次元多様体の構造を保ちながら表現する手法群。 |
| **Isomap** | 近傍グラフ上の最短路で測地距離を近似し、その距離を保つように埋め込みを求める手法。 |
| **LLE (Locally Linear Embedding)** | 各点を近傍点の線形結合で表す重みを推定し、その局所関係を保つように埋め込みを求める手法。 |
| **測地距離** | <a id="kw-geodesic-distance"></a>曲がった空間（多様体）上の最短経路に沿った距離。 |
| **局所線形性** | 多様体は十分小さい近傍では線形近似できる、というLLEの前提。 |
| **多次元尺度構成法 (MDS)** | 距離行列を保つように低次元配置を求める古典的手法。Isomapは測地距離行列にMDSを適用する。 |
| **Laplacian Eigenmaps** | 近傍グラフのラプラシアン固有ベクトルを用いて局所構造を保つ埋め込み手法。 |
| **out-of-sample 問題** | 標準的なIsomap/LLEで、学習後に新規点の座標を直接求めにくい課題。 |
...
| **TF-IDF** | 単語頻度（TF）と逆文書頻度（IDF）を掛け合わせ、頻出語の影響を抑えて語の重要度を重み付けする古典的手法。 |
...
| **共起統計** | 単語同士が同一文脈に現れる頻度パターンから関係性を捉える統計的アプローチ。 |
| **分布仮説** | 意味が近い単語は似た文脈に出現する、という自然言語処理の基本仮説。 |
| **PMI (点互情報量)** | 2語の共起確率が独立仮定からどれだけずれるかを測る尺度。共起の強さを定量化する。 |
| **PPMI** | PMIの負値を0に切った実用的変種。低頻度ノイズの影響を和らげつつ関連語を強調する。 |
...
| **情報量** (Self-information / Surprisal) | <a id="kw-self-info"></a> $-\log p(x)$ で定義される、ある出来事の「珍しさ」の尺度。確率1で0、確率0で無限大。クロスエントロピー損失の正体であり、エントロピーはその期待値（第7回）。 |
| **最大エントロピー原理** | 与えられた制約下で最もランダム（仮定が少ない）な分布を選ぶ原理。Softmaxが指数型になる理論的根拠。 |
| **境界の呪い** | 推定分布が確率単体の境界（確率0）に張り付くと、真の事象に0確率を割り当てた瞬間に損失が発散し得る問題。 |
| **n-gramモデル** | 前方の $n-1$ 語の文脈から次語確率を推定する古典的言語モデル。最尤推定では未観測語に0確率を与えやすい。 |
| **負の対数尤度 (NLL)** | 観測データの尤度に $-\log$ を取った損失。確率0を割り当てると $-\log 0 = \infty$ となる。 |
| **Laplace平滑化** | カウントに定数（典型的には $+1$ ）を加えて0確率を避ける古典的平滑化手法。 |
| **Kneser-Ney平滑化** | 割引とバックオフを組み合わせ、特に言語モデルで高精度な古典的平滑化手法。 |
| **データ疎性** | 観測されない事象が多く、最尤推定だけでは安定な確率推定が難しくなる問題。 |
| **Label Smoothing** | 正解ラベルをわずかに平滑化し、過度な確信や過学習を抑える正則化手法。 |
...
| **リー群** (Lie Group) | 回転や並進のような連続変換が、群構造と滑らかな多様体構造を同時に持つ数学的対象。RoPEの回転作用の背景にある。 |
| **SO(2)** | 2次元回転全体からなる特殊直交群。円周 $S^1$ と同型で、RoPEの各次元ペアで使う回転行列の自然な舞台。 |
...
| **集中度** | → [［再掲・第7回］](#kw-kappa) |
| **分布埋め込み** | データを1点に固定せず、確率分布（霧）として配置することで不確実性を明示的に扱う設計。 |
...
| **典型帯** (Typical Band) | 典型集合のうち、半径 $\sqrt{d}$ 付近に現れる薄い半径方向の帯。ガウス潜在空間の「居住可能帯」として現れる。 |
...
| キーワード | 説明 |
| --- | --- |
| **エントロピー** | <a id="kw-entropy"></a>分布の広がり（不確実性）を測る指標。情報量 $-\log p(x)$ の期待値 $H(P) = \mathbb{E}[-\log p(x)]$ として定義される。vMFでは $\kappa$ と次元の関数になる。 |
| **情報量** | → [［再掲・第4回］](#kw-self-info) |
...
| **測地距離** | → [［再掲・Appendix 4］](#kw-geodesic-distance) |
...
| キーワード | 説明 |
| --- | --- |
| **フィンスラー計量** (Finsler Metric) | 点ごとに一般の凸な単位球を許す計量。リーマン計量より柔軟に方向依存の「長さ」を表現できる。 |
| **非可逆フィンスラー計量** | $F(x,v) \neq F(x,-v)$ を許すフィンスラー計量。向きによって距離が変わる非対称性を距離そのものに埋め込める。 |
| **シンプレクティック形式** (Symplectic Form) | 位相空間上の非退化な閉2-形式。ハミルトン系の保存構造（2-形式保存）の基盤。 |
| **ハミルトン系** | ハミルトニアンにより時間発展が定まる力学系。理想化した連続時間では保存量と幾何学的構造が結びつく。 |
| **シンプレクティック積分器** | 離散時間計算でシンプレクティック構造を保つ数値積分法。長期シミュレーションでドリフトを抑えやすい。 |
| **Hamiltonian Neural Networks (HNN)** | ハミルトニアンをニューラルネットで学習し、力学系の保存的構造を取り入れるモデル。 |
...
| **双対アフィン接続** | Fisher計量に関して e-接続と m-接続が互いに双対をなす構造。単に「双対接続」とも。 |
...
| キーワード | 説明 |
| --- | --- |
| $\alpha$ -接続 | e-接続（ $\alpha=+1$ ）とm-接続（ $\alpha=-1$ ）を含む連続族の接続。 $\alpha$ と $-\alpha$ はFisher計量に関して双対。 |
| **Levi-Civita接続** ( $\alpha=0$ ) | 計量整合性と捩率なしを同時に満たす一意な接続。自己双対だが、統計多様体では一般に平坦でない。 |
| **スキューネス・テンソル** (Skewness Tensor) | $\alpha$ -接続を特徴づける3次テンソル（立方形式）。統計多様体の非ユークリッド的な歪みを表す。 |
| **計量整合性** (Metric Compatibility) | 平行移動で内積（長さ・角度）が保存される性質。一般に $\alpha \neq 0$ では失われる。 |
| **捩率なし** (Torsion-Free) | 接続が「ねじれ」を持たない条件。Levi-Civita接続を特徴づける要件の1つ。 |
| **e-平坦 / m-平坦** | e-接続では自然パラメータ、m-接続では期待値パラメータがアフィン座標となり、測地線が直線で表せる性質。 |
| **ブレグマン・ダイバージェンス** | 双対平坦空間でKLダイバージェンスを統一的に捉える枠組み。情報射影や分解則の基盤となる。 |
| **情報幾何のピタゴラス定理** | 双対平坦空間で成り立つKL/Bregmanの直交分解（Pythagorean relation）。射影問題の幾何学的理解を与える。 |
...
| キーワード | 説明 |
| --- | --- |
| **特異モデル** (Singular Model) | パラメータと分布の対応が多対1になり、同定不能性を持つモデル。深層学習モデルの多くが該当する。 |
| **正則モデル** (Regular Model) | パラメータと分布が局所的に1対1で対応し、通常の漸近理論（BIC等）が成り立つモデル。 |
| **特異点** (Singularity) | パラメータ空間でFisher情報行列が退化（ランク落ち）する点。損失地形の平坦化や分岐の原因となる。 |
| **同定不能性** (Non-identifiability) | 異なるパラメータが同一の分布・関数を表してしまう性質。 |
| **ランク落ち** | Fisher情報行列や重み行列の実効ランクが下がり、冗長な自由度が発生する現象。 |
| **代数多様体** | 特異点を含みうる幾何学的対象。特異モデルのパラメータ空間を捉える際の基盤概念。 |

### 汎化を決める量

| キーワード | 説明 |
| --- | --- |
| **特異学習理論** (SLT) | 渡辺澄夫らが体系化した、特異モデルのベイズ漸近理論。汎化と学習曲線を幾何学的に説明する。 |
| **ベイズ自由エネルギー** | 周辺尤度の負の対数。モデル複雑さと当てはまりを統合して評価する量。 |
| **RLCT** (Real Log Canonical Threshold) | <a id="kw-rlct"></a>自由エネルギーの $\log n$ 項係数として現れる幾何学的不変量。特異モデルの実質的複雑さを表す。 |
| **重複度** (Multiplicity) | 学習ゼータ関数の極の重複度。特異モデルで現れる $\log \log n$ 項の係数に関与する。 |
| **WBIC** | 特異モデルでも使えるベイズ情報量基準。BICの特異モデル拡張として用いられる。 |
| **有効次元** | パラメータ数ではなく、谷の幾何により決まる実質的な次元。SLTではRLCTと強く関係する。 |

### 学習ダイナミクスと幾何学的操作

| キーワード | 説明 |
| --- | --- |
| **Flat Minima** | 周囲に広い体積を持つ平坦な最小解。SLTでは汎化と結びつく幾何学的対象として位置づけられる。 |
| **相転移** (Phase Transition) | 学習中に停滞期（プラトー）から急激に性能が変わる現象。特異点近傍の遷移として解釈できる。 |
| **プラトー** | 特異点近傍の平坦な領域で学習が一時停滞する挙動。 |
| **Grokking** | 長い停滞後に突然汎化が進む現象。SLTでは相転移的挙動として分析される。 |
| **特異点解消** (Resolution of Singularities) | ブローアップ等により特異点を滑らかな座標系へ持ち上げ、解析可能な形に変換する手法。 |
| **ブローアップ** (Blow-up) | 特異点を方向ごとに展開して交差・退化をほどく代数幾何の基本操作。 |
