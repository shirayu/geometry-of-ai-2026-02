...
| **自然勾配** | パラメータ空間の距離ではなく、確率分布間の「区別のしやすさ（フィッシャー情報計量）」に基づく最急降下方向。 |
| **フィッシャー情報行列** | 確率分布の空間におけるリーマン計量。パラメータの変化が分布に与える影響度を定量化する。 |
...
| **集中度** ( $\kappa$ ) | 分布の凝縮度合い。大きいほど「確実（点に近い）」、小さいほど「不確実（霧に近い）」ことを示す。 |
| **分布埋め込み** | データを1点に固定せず、確率分布（雲）として配置することで不確実性を明示的に扱う設計。 |
| **VAE (Variational Autoencoder)** | 潜在変数を分布として扱い、ELBO最適化で学習する生成モデル。潜在空間の不確実性を明示化する。 |
...
| **回転不変量** | 座標系の回転に左右されない統計量。  $\kappa$ を使うことで、モデル間の整列（Alignment）なしでの比較が可能になる。 |
...
| **ベクトル場** | 各点に対して「次にどの方向に動くべきか」を指示する関数。ニューラルネットワークの本質的役割。 |
...
これまでの講義を支える、曲がった空間の数学的基礎。

| キーワード | 説明 |
| --- | --- |
| **リーマン計量** | 多様体上の各点において、距離や角度を定義するための「ものさし（正定値対称行列）」。 |
| **測地線** | 曲がった空間における2点間の最短経路。ニュートンの第一法則（等速直線運動）の一般化。 |
| **指数写像** | 接空間（平らな近似）上のベクトルを、多様体上の点へと「巻き付ける」操作。局所的な座標系を与える。 |
...
| **TDA (トポロジカルデータアナリシス)** | 位相幾何学の道具を用いて、高次元データの複雑な形状や異常（穴の欠損など）を分析する手法。 |
...
| **ルーティング** | 入力依存で接続やExpertを選ぶ機構。 |

### スパース化とMoE

| キーワード | 説明 |
| --- | --- |
| **Sparse Attention (Top-K)** | 重要な接続のみを選び、計算量を減らす注意機構。 |
| **MoE (Mixture of Experts)** | 入力ごとに部分モデルを選択する疎構造アーキテクチャ。 |
...
| **コサイン類似度** | ノルムを無視し、方向の近さのみを測る尺度。 |

### 非対称な尺度（情報の隔たり）

| キーワード | 説明 |
| --- | --- |
| **KLダイバージェンス** | 分布の隔たりを測る非対称な尺度。 |
| **フィッシャー情報行列** | 分布の変化しやすさを定める情報幾何の計量。 |
