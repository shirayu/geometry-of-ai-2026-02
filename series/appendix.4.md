...
| --- | --- | --- | --- | --- |
| **表層** | データ | $x$ | 分布から落とした「影・雨粒」 | 第0回 |
| **中層（点）** | 情報量 | $-\log p(x)$ | 確率空間の「高さ（到達コスト）」 | 第4回 |
| **中層（分布）** | エントロピー | $H(P)$ | 分布という「霧」の体積 | 第7回 |
| **深層** | フィッシャー情報 | $F(\theta)$ | 違いを区別できる能力（感度）。霧の形が変わりやすい場所ほど遠い。 | 本Appendix |

「データ」は表層に過ぎない。その背後には「どれだけ珍しいか（情報量）」「どれだけ広がっているか（エントロピー）」「どれだけ区別できるか（フィッシャー情報）」という、より深い構造が潜んでいる。幾何学とは、これら4層を同一の「空間の言葉」で語る試みである。
