...
> [!NOTE]
> **歪みの定義：** グラフ上の最短経路距離 $d_G(u,v)$ と、埋め込み空間での距離 $d_E(f(u), f(v))$ の比率の最大値と最小値の比を**歪み**と呼ぶ。歪みが1に近いほど、元の距離関係が正確に保存されている。Sarkar (2011) は、$n$ ノードの木をユークリッド空間に歪み $O(1)$ で埋め込むには、$\Omega(\log n)$ 次元が必要であることを示した。

### 球面空間での困難

では、球面 $S^{d-1}$ ではどうか。

球面は正の曲率を持つ。これは、「周囲長が $\sin(r)$ に比例する」ことを意味する。ここで $r$ は球面上の距離（測地線距離）である。
...
| 空間 | 曲率 | 周囲長の成長 | 木構造への適性 |
| --- | --- | --- | --- |
| ユークリッド | 0 | $\propto r^{d-1}$（多項式的） | 低い（次元爆発） |
| 球面 | 正 | $\propto \sin(r)$（有界） | 低い（容量不足） |
| 双曲 | 負 | $\propto \sinh(r) \approx e^r$（指数的） | **高い** |
...
> [!NOTE]
> **局所的な形状と大域的な構造：** 鞍型曲面は負の曲率の「局所的な形状」を示すが、双曲空間全体を3次元ユークリッド空間に埋め込むことはできない（Hilbertの定理）。そのため、双曲空間を視覚化するには「モデル」が必要になる。これは、球面を平面の地図で表すときに歪みが生じるのと似ている。
...
双曲空間は、3次元ユークリッド空間には埋め込めない。そのため、私たちは双曲空間を「見る」ために、何らかの**モデル（model）** を使う必要がある。
...
2. **低次元**：ユークリッド空間では $O(\log n)$ 次元必要だったものが、双曲空間では2次元でも十分な場合がある
...
ここで、球面（正曲率）と双曲空間（負曲率）の性質を改めて比較しよう：

| 性質 | 球面（正曲率） | 双曲空間（負曲率） |
| --- | --- | --- |
| 三角形の内角和 | > 180° | < 180° |
| 周囲長の成長 | $\sin(r)$（有界） | $\sinh(r) \approx e^r$（指数的） |
