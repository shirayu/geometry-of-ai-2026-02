...
$-\log p(x)$ は **情報量（self-information / surprisal）** と呼ばれる量である。幾何学的には、これは確率空間における「高さ（標高）」に対応する：

- $p = 1$ （確実な出来事） $\implies -\log p = 0$ （地面、高さゼロ、驚きなし）
- $p \approx 0$ （希少な出来事） $\implies -\log p \to \infty$ （成層圏、高さ無限大、大きな驚き）

深層学習が損失を最小化して「山を駆け下りる」とき、我々が減らしているのはモデルが正解を「どれだけ珍しいことだと見なすか」という高さである。言い換えれば、**正解クラスをできるだけ「地面（当然のこと）」として扱えるよう、モデルのパラメータを動かしている**。

エントロピー $H(P) = \mathbb{E}_{x \sim P}[-\log p(x)]$ は、この「高さ」の分布 $P$ 全体に対する平均値である。第7回では、このエントロピーが分布という「霧の体積」として再登場する。
