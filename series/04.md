...
$$p_t(x) = \exp((1-t)\boldsymbol{\theta}_0 + t\boldsymbol{\theta}_1)^\top \mathbf{t}(x) - \psi((1-t)\boldsymbol{\theta}_0 + t\boldsymbol{\theta}_1))$$

Softmaxで言えば、**ロジット空間 $\mathbb{R}^K$ での直線が、そのまま e-接続の測地線に対応する**。
...
これは m-接続の測地線となる。確率単体 $\Delta^{K-1}$ をユークリッド空間の部分集合と見たときの通常の直線が、この測地線に対応する。
...
| --- | --- | --- | --- |
| 自然パラメータ $\boldsymbol{\theta}$ | e-平坦 | $\boldsymbol{\theta}(t) = (1-t)\boldsymbol{\theta}_0 + t\boldsymbol{\theta}_1$ | ロジット空間での直線 |
| 期待値パラメータ $\boldsymbol{\eta}$ | m-平坦 | $\boldsymbol{\eta}(t) = (1-t)\boldsymbol{\eta}_0 + t\boldsymbol{\eta}_1$ | 確率単体での直線 |
...
**Softmaxとの関係**：
カテゴリカル分布において：
- 自然パラメータ = ロジット $\mathbf{z}$ 
- 期待値パラメータ = 確率 $\mathbf{p}$ （より正確には、指示関数の期待値）
- ルジャンドル変換 = Softmax変換とその逆のlog変換

Softmaxの背後には、この双対平坦構造が隠れている。Softmaxが「自然な」変換であるのは、指数型分布族の双対構造が要請する写像だからである。
...
> [!IMPORTANT]
> **なぜ双対平坦性が重要か**：
> 1. **KLダイバージェンスの分解**：双対平坦空間では、KLダイバージェンスが「ピタゴラスの定理」のような美しい分解公式（三平方の定理の一般化）を持つ。これが情報射影や変分推論の幾何学的理解につながる。
> 2. **自然勾配の計算効率**：指数型分布族では、自然パラメータと期待値パラメータの間の変換が簡潔であり、Fisher情報行列の逆行列が解析的に扱いやすい。
> 3. **座標の選択の指針**：機械学習において「どのパラメータ表現を使うか」は性能に影響する。双対平坦構造は、この選択に対する理論的な指針を与える。
