...
> [!TIP]
> **なぜ確率を直接扱わないのか：** ニューラルネットワークで確率 $\mathbf{p} \in \Delta^{K-1}$ を直接出力しようとすると、いくつかの問題が生じる。(1) **制約の複雑さ**： $\sum_i p_i = 1, p_i \geq 0$ という制約を満たしながら勾配降下法を適用するのは面倒である。制約付き最適化や射影演算が必要になる。(2) **線形変換との不整合**：ネットワークの最終層は通常 $\mathbf{z} = W\mathbf{x} + \mathbf{b}$ という線形変換であるが、この出力は $\mathbb{R}^K$ 全体を取りうる。線形変換の出力が自動的に単体の制約 $\sum_i p_i = 1, p_i \geq 0$ を満たす保証はないため、線形変換の出力を直接確率として解釈できない。(3) **数値的な扱いづらさ**：確率が0に近いとき、勾配が不安定になりやすい。一方、ロジット空間 $\mathbb{R}^K$ は制約のない全空間であり、線形変換の出力をそのまま受け取れる。Softmaxは、この「自由な実数空間」から「制約された確率空間」への滑らかな橋渡しを担う。つまり、**ネットワークは自由に予測し、Softmaxが事後的に整合性を保証する**という分業が、実装と学習の両面で効率的なのである。
