...
> [!NOTE]
> **ロジット（logit）とは：** ロジット $z_i$ とは、確率 $p_i$ に変換される前の「生のスコア」であり、任意の実数値を取りうる。名前の由来は、2クラス分類における**ロジスティック関数**（シグモイド関数） $p = \sigma(z) = \frac{1}{1 + e^{-z}}$ の逆関数 $z = \sigma^{-1}(p) = \log\frac{p}{1-p}$ が「**log**-odds（対数オッズ）」と呼ばれることから、その短縮形として**logit**と命名された。多クラス分類では、Softmaxへの入力ベクトル $\mathbf{z}$ 全体をロジットベクトルと呼ぶ。
> 
> **なぜ確率を直接扱わないのか：** ニューラルネットワークで確率 $\mathbf{p} \in \Delta^{K-1}$ を直接出力しようとすると、いくつかの問題が生じる。(1) **制約の複雑さ**： $\sum_i p_i = 1, p_i \geq 0$ という制約を満たしながら勾配降下法を適用するのは面倒である。制約付き最適化や射影演算が必要になる。(2) **線形性の喪失**：確率は凸集合（単体）内に閉じ込められるため、ネットワークの最終層が線形変換 $\mathbf{z} = W\mathbf{x} + \mathbf{b}$ であっても、その出力を直接確率として解釈できない。(3) **数値的な扱いづらさ**：確率が0に近いとき、勾配が不安定になりやすい。一方、ロジット空間 $\mathbb{R}^K$ は制約のない全空間であり、線形変換の出力をそのまま受け取れる。Softmaxは、この「自由な実数空間」から「制約された確率空間」への滑らかな橋渡しを担う。つまり、**ネットワークは自由に予測し、Softmaxが事後的に整合性を保証する**という分業が、実装と学習の両面で効率的なのである。
