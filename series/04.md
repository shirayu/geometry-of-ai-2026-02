...
> [!CAUTION]
> 数式から明らかだが、 **どの次元の値も、数理上は決して0にも1にもならない**。コンピュータでは数値表現の限界から0や1に丸められうるが、これはSoftmaxの重要な構造的性質である（後述）。
...
1. 負のロジットも正の確率に変換できる
2. 微分可能で滑らか
3. 後述する最大エントロピー原理との整合性

ただし、指数関数は数値的にオーバーフローしやすいため、実装では最大値を引く安定化が必要（実装ノート参照）。
...
> [!TIP]
> **「境界の呪い」とSoftmaxの功績**: 上述の「Softmaxは決して0にならない（常に単体の内部に留まる）」という性質は、自然言語処理の歴史において、実は「計算の破綻」を防ぐための決定的な役割を果たしている。
>
> **1. 境界の呪い（無限大の損失）**
> かつての素朴なn-gramモデル（最尤推定）では、学習データに無い単語の確率は $0$ になる。これは幾何学的に、分布が単体の「境界」に張り付く状態だ。もし推定された分布が正解データに対して確率 $0$ を割り当ててしまうと、負の対数尤度は $-\log 0 = \infty$ となり、学習や評価が破綻する。
> （KLダイバージェンスの定義において、 $P_{\text{予測}}(x)=0$ かつ $P_{\text{真}}(x)>0$ となる点が一つでもあると、対数項 $\log (P_{\text{真}}/P_{\text{予測}})$ が無限大に発散してしまうためだ。）
>
> **2. 構造による解決**
...
>
> 現代でもデータ疎性の問題自体は残るため（その対策の一例としてLabel Smoothing等が使われる）、統計的な課題が消えたわけではない。しかし、Softmaxのおかげで、少なくとも「たった一箇所の確率0で損失が無限大になり、計算全体が破綻する」という悪夢からは解放され、安心してエンドツーエンド学習が行えるようになったのである。

### なぜSoftmaxは確率を直接扱わずにロジットを使うのか

ニューラルネットワークで確率 $\mathbf{p} \in \Delta^{K-1}$ を直接出力しようとすると、いくつかの問題が生じる。

1. 制約の複雑さ

    $\sum_i p_i = 1, p_i \geq 0$ という制約を満たしながら勾配降下法を適用するのは面倒で、制約付き最適化や射影演算が必要になる。

2. 線形変換との不整合

    ネットワークの最終層は通常 $\mathbf{z} = W\mathbf{x} + \mathbf{b}$ という線形変換であり、その出力は $\mathbb{R}^K$ 全体を取りうる。この出力が自動的に単体の制約を満たす保証はないため、直接確率として解釈できない。

3. 数値的な扱いづらさ

    確率が0に近いとき、勾配が不安定になりやすい。一方、ロジット空間 $\mathbb{R}^K$ は制約のない全空間であり、線形変換の出力をそのまま受け取れる。

Softmaxは、この「自由な実数空間」から「制約された確率空間」への滑らかな橋渡しを担う。**ネットワークは自由に予測し、Softmaxが事後的に整合性を保証する**という分業が、実装と学習の両面で効率的なのである。
...
損失関数 $\mathcal{L} = -\log p_y$ に対数が現れる理由を、幾何学的に問い直してみよう。

クラス $c$ に対して $-\log p_c$ で定義される量を **情報量（self-information / surprisal）** と呼ぶ。これは「確実性からの遠さ」として捉えることができる：

- $p_c = 1$ （確実な出来事） $\implies -\log p_c = 0$ （遠さゼロ、驚きなし）
- $p_c \approx 0$ （希少な出来事） $\implies -\log p_c \to \infty$ （遠さ無限大、大きな驚き）

損失 $\mathcal{L} = -\log p_y$ を最小化することは、正解クラス $y$ を「遠い場所」から「手元（ $p_y \to 1$ ）」へ引き寄せることに相当する。言い換えれば、**正解クラスをできるだけ「当然のこと（遠さゼロ）」として扱えるよう、モデルのパラメータを動かしている**。

エントロピー $H(P) = \mathbb{E}_{c \sim P}[-\log p_c]$ は、この「遠さ」の分布 $P$ 全体に対する平均値である。第7回では、このエントロピーが分布という「霧の体積」として再登場する。
