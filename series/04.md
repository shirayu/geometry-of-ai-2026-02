...
逆向きは $z_i = \log p_i + C$ （ $C$ は任意定数）となり、前述の通り $\mathbf{1}$ 方向の不定性が残る。この不定性こそが、Softmaxが商空間を経由する構造の現れである。
...
> 2. **自然勾配の座標不変性**：自然勾配は、Fisher計量に関して **座標系によらず最急降下** を実現する。指数型分布族では自然パラメータと期待値パラメータの間の変換が簡潔であり、Fisher情報行列の逆行列が解析的に扱いやすい（例：カテゴリカル分布では対角化される）。
