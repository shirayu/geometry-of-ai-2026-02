...
> [!NOTE]
> **ハードマージンとソフトマージン：** 上記の定式化は **ハードマージンSVM** であり、データが完全に線形分離可能であることを前提とする。現実のデータでは分離不可能な場合が多いため、**ソフトマージンSVM** が使われる。ソフトマージンでは、スラック変数 $\xi_i \geq 0$ を導入し、制約を $y_i(\mathbf{w}^\top \mathbf{x}_i + b) \geq 1 - \xi_i$ に緩和する。目的関数は $\min_{\mathbf{w}, b, \boldsymbol{\xi}} \frac{1}{2}\|\mathbf{w}\|^2 + C \sum_i \xi_i$ となり、正則化パラメータ $C$ がマージン最大化（第1項）と誤分類許容（第2項）のトレードオフを制御する。
...
> [!NOTE]
> **大円になる条件：** 境界が大円になるのは、決定境界が $\mathbf{w}^\top \mathbf{x} = 0$ のように原点を通る超平面で表される場合である。ArcFaceのように正規化された特徴と重みの内積（バイアスなし）で分類を行う設計では、各クラス間の境界は原点を通る超平面と球面の交線、すなわち大円になる。一般にバイアス $b$ を含めると、境界は $\{\mathbf{x} : \mathbf{w}^\top \mathbf{x} + b = 0, \|\mathbf{x}\| = 1\}$ となり、球面上では大円ではなく小円になる。
