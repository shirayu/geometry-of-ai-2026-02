...
CLIPのような複雑な対照学習を論じる前に、異なる「世界」を接続する最もシンプルな方法の一つを振り返ってみよう。

2007年、Hal Daumé IIIが提案した**FEDA（Frustratingly Easy Domain Adaptation）**は、ドメイン適応という課題に対して、驚くほど単純な「空間の拡張」で解決を図った。

当時、ドメイン適応の研究は複雑なカーネル法や特異値分解を駆使した高度な手法が主流だった。しかしFEDAは、**特徴量を3倍にコピーして繋げるだけ**という、たった数行で実装できる手法で、これら複雑な手法を凌駕してしまったのである。

### 幾何学的な本質：次元の直交化

FEDAの核心は、元の $d$ 次元の特徴量 $\mathbf{x}$ を、**$3d$ 次元の拡張空間**へ写像することである：
...
2. **第2ブロック（ソース専用）**：ソース固有の性質を「逃がす」ための直交した軸
3. **第3ブロック（ターゲット専用）**：ターゲット固有の性質を「逃がす」ための直交した軸

これは、**共通の性質と個別の性質を幾何学的に直交させる**という、極めてクリアな設計指針である。
...
FEDAの精神は、名前こそ表に出る機会は減ったが、現代の最先端技術の中に「遍在」している：

**LoRA（Low-Rank Adaptation）**：巨大な事前学習モデル（Frozen）の横に、小さな「差分空間（Adapter）」を並列に用意する。これは「元の汎用的な知識（Common）」を壊さずに、「特定のタスク固有の知識（Specific）」を**別次元の計算パスとして追加する**という、FEDAと同じ幾何学的戦略である。

**Mixture of Experts（MoE）**：第13回で扱うこの手法も、「共有知識」と「専門知識」を分離するという点で、FEDAの思想を継承している。

**モダリティ専用トークン**：ImageBindのような現代的なマルチモーダルモデルでも、「共有埋め込み」と「モダリティ専用の表現」を分離する設計が見られる。

> [!IMPORTANT]
> **「空間の作り方」の重要性：** FEDAが2007年の論文でありながら、現在までに2,000件以上引用され続けているのは、**「モデルを複雑にするのではなく、空間（多様体）の定義を変えることで問題を解決する」**という普遍的な設計原理を示したからである。これは本講義が一貫して主張する「幾何学的視点」の原点とも言える。
...
# 直交性の確認：ソース専用ブロックとターゲット専用ブロックは互いに0
source_block = expanded_features[:, dim:2*dim]
target_block = expanded_features[:, 2*dim:]
orthogonality = (source_block * target_block).sum() / batch_size
print(f"Orthogonality check (should be ~0): {orthogonality.item():.6f}")
