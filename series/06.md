...
> [!NOTE]
> **リー群への一瞥 ── 「対称性の幾何」：** RoPEで使われている回転行列 $R_m$ の全体は、数学的には **リー群（Lie Group）** と呼ばれる構造を持つ。リー群とは、回転や平行移動などの「対称変換」の集合が、それ自体として滑らかな多様体になっているものを指す。2次元の回転全体 $\text{SO}(2)$ は円周 $S^1$ と同じ形をしており、RoPEの各次元ペアの回転はまさにこの $\text{SO}(2)$ 上の「動き」である。
>
> リー群の重要な性質は、**群の演算（合成・逆元）が滑らかであること**だ。RoPEにおいて $R_m^\top R_n = R_{n-m}$ が成り立つのは、$\text{SO}(2)$ の群構造そのものである。回転の合成がまた回転になり、逆回転も回転であるという、閉じた代数構造が「相対位置だけが残る」という性質を保証している。
>
> 本講義では、リー群の理論に深入りする必要はない。しかし、「動き（変換）自体が幾何学的な空間をなす」という視点は、RoPEの設計がなぜ数学的にうまくいくのかを理解する鍵となる。回転という操作が、単なる計算テクニックではなく、対称性という深い数学的構造に根ざしていることを知っておくだけで十分である。
