...
$$e_\tau^{\text{in}}: \mathcal{X}_\tau \to \Sigma^*, \quad e_\tau^{\text{out}}: \mathcal{Y}_\tau \to \Sigma^*$$

を用意し、学習は $\Sigma^* \to \Sigma^*$ という単一の写像で行うことと見なせる。ここで $\Sigma^*$ は文字列（トークン列）の空間である。

重要なのは、これが「すべてのタスクを完全に統一した」というより、**共通のインタフェース（入出力形式）を提供した**ことである。タスク間の性能差や相性は依然として存在するが、少なくとも「同じ言語で表現できる」形に寄せたことで、転移学習がしやすくなった。
...
この哲学的転換により、Transformerは**統一された表現空間**を持つようになった。タスク間の転移学習が容易になり、翻訳で学んだ知識が要約に転移し、質問応答で学んだ知識が感情分析に転移しやすくなる。すべてが同じ入出力形式で表現されるからだ。

ただし、これは「タスクの壁が完全に消えた」というより、**転移しやすい形に寄せた**と理解すべきである。タスク間の性能差や相性は依然として存在する。
...
Attention機構という「動的な測量システム」は、T5という「設計思想」によって、**統一された入出力形式を持つ表現空間**を実現した。固定された地図（カーネル法）でもなく、タスクごとに異なる地図（初期の事前学習モデル）でもなく、**一つの測量システムで多様な地形を同じ形式で記述できる**世界が開かれた。
...
**T5は、Transformerの「仕組み」を変えずに「設計思想」だけで到達できる地平を示した。** すべてのタスクを「テキスト生成」という統一された入出力形式に寄せることで、タスク間の転移学習が容易になった。これは、適切な座標系（表現形式）の選択が問題を劇的に簡潔にする例である。
