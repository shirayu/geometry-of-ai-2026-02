...
[^geodesic]: リーマン幾何学において測地線は変分原理（長さ汎関数の第一変分＝0）で定義される局所的な性質であり、大域的な最短経路を保証するものではない。例えば球面上で2点を結ぶ大円は2本あるが、長い方も測地線である。ただし十分に近い2点間では、測地線は最短距離を与える。
...
ただし、これは「欠点」ではなく **双対性の帰結** として理解すべきである。$\alpha$-接続は計量整合性を手放す代わりに、$\alpha$ と $-\alpha$ の間の双対性を満たす。Levi-Civita接続（$\alpha = 0$）が自己双対として計量整合性と双対性を両立できるのは、$\alpha = -\alpha$ が成り立つ唯一の点であるからに過ぎない。
...
通常の多様体は一般に曲がっているが、**正則な（regular）** 指数型分布族の空間は：
...
この2つの平坦性が **同時に成り立つ** 空間を **双対平坦空間（dually flat space）** と呼ぶ[^regularity]。

[^regularity]: ここでの議論は、指数型分布族が **正則（regular）** かつ **最小表現（minimal representation）** である場合を前提としている。正則性は $\boldsymbol{\theta}$ の自然パラメータ空間が $\mathbb{R}^d$ の開集合であること、最小性は十分統計量 $\mathbf{t}(x)$ の成分間にアフィン従属な関係がないことを要請する。カテゴリカル分布を含む標準的な指数型分布族はこの条件を満たすが、パラメータが曲面上に制約される **曲指数型分布族（curved exponential family）** では双対平坦性は一般に成り立たない。
...
> この勾配 $\nabla_{\boldsymbol{\eta}} \phi = (\log \eta_1 + 1, \dots, \log \eta_K + 1)^\top$ は $\log \boldsymbol{\eta}$ に定数を加えたものを返す。自然パラメータ $\boldsymbol{\theta}$ の代表元を得るには、ゲージ固定（例えば $\sum_i \theta_i = 0$ ）を課す必要がある。すなわち、**勾配が直接返すのは $\log p$ に定数を加えた量であり、商空間の代表元としての $\boldsymbol{\theta}$ はゲージ固定で初めて一意に定まる**。
>
