...
def visualize_trajectory_2d(trajectory, title="State Trajectory"):
    ...
    for i in range(min(batch_size, 20)):  # 最大20サンプル
        ...
        ax.scatter(traj_2d[0, i, 0], traj_2d[0, i, 1], 
                   color=colors[i], marker='o', s=50, label='Start' if i == 0 else '')
        ax.scatter(traj_2d[-1, i, 0], traj_2d[-1, i, 1], 
                   color=colors[i], marker='x', s=50, label='End' if i == 0 else '')
